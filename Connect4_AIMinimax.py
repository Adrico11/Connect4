...
def minimax(board,depth,maximizingPlayer): #Considère 7**depth configuration possible (complexté exponentielle)
    ...
    if terminal_node(board):
        ...
        if maximizingPlayer:
            ...
            for col in valid_loc:
                ...
                new_value=minimax(board1,depth-1,False)[1] #On ne veut que le score
                if new_value>value:
                    value=new_value
                    best_col=col
                #board1[row][col]=0
            ...
            for col in valid_loc:
                ...
                new_value=minimax(board2,depth-1,True)[1]
                if new_value<value:
                    value=new_value
                    best_col=col      
            return best_col,new_value  
...
while not game_over:
    ...
    while turn==AI and not game_over: #L'IA choisi sa colonne
        piece=AI_PIECE
        col,minimax_score = minimax(board,4,True)#L'IA choisit une colonne
